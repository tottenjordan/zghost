        ...
        trends_list.append(
            {
                ...
                "description": (
                    video["snippet"].get("description", "")[:200] + "..."
                    if len(video["snippet"].get("description", "")) > 200
                    else video["snippet"].get("description", "")
                ),
                "duration": video["contentDetails"]["duration"],
        ...
        google_trends_list.append(
            {
                ...
                "rank": int(row["rank"]),
                "refresh_date": (
                    row["refresh_date"].strftime("%m/%d/%Y")
                    if hasattr(row["refresh_date"], "strftime")
                    else str(row["refresh_date"])
                ),
            }
        )
