...
async def generate_narrative_pdf(request: NarrativePdfRequest):
    """Generate a PDF from narrative/report content using MarkdownPdf (preserves images, links, formatting) and upload to GCS."""
    try:
        import tempfile
        from markdown_pdf import MarkdownPdf, Section
        from google.cloud import storage as gcs_storage
        ...
        report_type = request.report_type or "final"
        ...
        # Build PDF using MarkdownPdf — preserves markdown formatting, images, links
        pdf = MarkdownPdf(toc_level=4)
        pdf.add_section(Section(f" {content}\n"))
        prefix = "[Draft]" if report_type == "draft" else "[Final]"
        pdf.meta["title"] = f"{prefix} {title}"
        ...
        # Upload to GCS
        bucket_name = os.environ.get("BUCKET", "zghost-media-center").replace("gs://", "")
        type_label = "draft" if report_type == "draft" else "final"
        blob_name = f"reports/{request.session_id}/{type_label}_report_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.pdf"
        ...
        # Update session state with PDF URL
        state_key = "draft_pdf_url" if report_type == "draft" else "final_pdf_url"
        if app_state.session_service:
            try:
                session = await app_state.session_service.get_session(
                    app_name=SESSION_APP_NAME,
                ...
                if session:
                    session.state[state_key] = gcs_uri
            except Exception as e:
