...
(2) **Analyze Trending YouTube Videos** - Analyze user-selected trending YouTube video(s); gather related context through web search to understand why the video(s) are trending.
...
1. Follow all steps in the <Broad Market Research> section to conduct research on concepts from the `campaign_guide`
2. Follow all steps in the <Trending YouTube Videos> section to conduct research to better understand the context of trending YouTube videos
3. Follow all steps in the <Trending Search Terms> section to better understand the context of trending topics
4. Once all steps are complete, transfer back to the root agent.
...
2. To better understand any key entities mentioned in the video, Use the `query_web` tool to perform few Google Searches as needed.
...
2. Using the output from the previous step, generate trend context that includes a concise summary describing what is taking place or being discussed:
  * Describe any key entities (i.e., people, places, organizations, named events, etc.).
  * Describe the relationships between these key entities, especially in the context of the trending topic.
  * Explain if you think this trend will resonate with the `target_audience` described in the `campaign_guide`.
  * Suggest how this trending content could possibly relate to the {campaign_guide.target_product} in a marketing campaign. 
