...
echo -e "${GREEN}[1/3] Starting ADK web server on port 8000...${NC}"
uv run adk web trends_and_insights_agent --port 8000 &
PIDS+=($!)

# 2. Voice WebSocket server (port 8081)
echo -e "${GREEN}[2/3] Starting voice WebSocket server on port 8081...${NC}"
python voice_server.py &
PIDS+=($!)

# 3. Frontend dev server (port 5173)
echo -e "${GREEN}[3/3] Starting frontend dev server on port 5173...${NC}"
