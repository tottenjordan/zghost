...
visual generation, AV editing studio (30s commercial with character consistency),
and focus group evaluation.
...
Usage:
  ...
  python tests/ae_e2e_senior_citizens_test.py
"""
...
def check_character_consistency(remote_agent, session):
    """Check that commercial metadata includes character consistency info."""
    state = get_session_state(remote_agent, session)
    commercial = state.get("commercial_artifact", {})
    if not isinstance(commercial, dict):
        print(f"  [FAIL] commercial_artifact is not a dict")
        return False

    metadata = commercial.get("metadata", {})
    if not isinstance(metadata, dict):
        print(f"  [FAIL] commercial_artifact.metadata is not a dict")
        return False

    # Check for character consistency fields
    has_character = "character_description" in metadata or "subject_person" in metadata
    if has_character:
        print(f"  [PASS] commercial_artifact.metadata contains character consistency info")
        char_desc = metadata.get("character_description") or metadata.get("subject_person")
        if char_desc:
            print(f"    Character: {str(char_desc)[:100]}...")
        return True
    else:
        print(f"  [WARN] commercial_artifact.metadata missing character consistency fields")
        return False


def main():
    print("=" * 70)
    print("Agent Engine E2E Test: McDonald's / McRib & Shamrock Shake / Senior Citizens")
    ...
    # ===================================================================
    print("\n\n--- STEP 11: AV Editing Studio (30s Commercial with Character Consistency) ---")
    response, tools = send_message(
        remote_agent, session,
        (
            ...
            "to the McDonald's McRib and Shamrock Shake campaign for Senior Citizens. "
            "Use the full pipeline: generate subject images with consistent character, "
            "create 4 clip scenes with frame chaining for character continuity, "
            "concatenate, trim to 30 seconds, and save the commercial artifact with "
    ...
    if not commercial_saved:
        ...
        response, tools = send_message(
            ...
            (
                ...
                "with the final video and metadata including character consistency info."
            ),
            timeout=EXTRA_LONG_TIMEOUT,
        )
        commercial_saved = check_state_key(remote_agent, session, "commercial_artifact")
    ...
    if commercial_saved:
        ...
        check_character_consistency(remote_agent, session)
    ...
    if not analyze_called:
        ...
        response, tools = send_message(
            remote_agent, session,
            (
                "Please provide the complete focus group evaluation including:\n"
                "- Overall score (1-10)\n"
                "- Individual panelist reactions and scores\n"
    ...
    # Check for focus group scoring in response
    response_lower = (response or "").lower()
    has_scoring = any(indicator in response_lower for indicator in ["score", "rating", "/10", "out of 10"])
    has_recommendation = any(indicator in response_lower for indicator in ["go/no-go", "recommendation", "proceed", "approve"])

    print(f"  [{'PASS' if has_scoring else 'WARN'}] Focus group response includes scoring: {has_scoring}")
    print(f"  [{'PASS' if has_recommendation else 'WARN'}] Focus group response includes Go/No-Go: {has_recommendation}")
    ...
    character_consistency = check_character_consistency(remote_agent, session)
    ...
    print(f"  [{'PASS' if character_consistency else 'WARN'}] commercial_artifact character consistency metadata")
    ...
    if all_pass and img_pass and vid_pass and commercial_pass:
        ...
        print(f"  Character consistency: {'YES' if character_consistency else 'NO'}")
        print(f"  Focus group scoring: {'YES' if has_scoring else 'NO'}")
        print(f"  Focus group Go/No-Go: {'YES' if has_recommendation else 'NO'}")
