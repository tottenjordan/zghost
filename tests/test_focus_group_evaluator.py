...
video analysis tool, save_focus_group_evaluation tool, and root_agent integration.
"""
...
class TestFocusGroupAgentConfiguration:
    ...
    def test_agent_has_save_evaluation_tool(self):
        from trends_and_insights_agent.skills.focus_group.agents import (
            focus_group_evaluator_agent,
        )
        tool_names = {
        ...
        assert "save_focus_group_evaluation" in tool_names
...
class TestFocusGroupPrompt:
    ...
    def test_prompt_references_save_tool(self):
        prompt = self._get_prompt()
        assert "save_focus_group_evaluation" in prompt

    def test_prompt_references_iteration_number(self):
        prompt = self._get_prompt()
        assert "iteration_number" in prompt
        assert "focus_group_iteration" in prompt
...
# ===================================================================
# 4. Save Focus Group Evaluation Tool
# ===================================================================
class TestSaveFocusGroupEvaluationTool:

    def _import_tool(self):
        from trends_and_insights_agent.skills.focus_group.tools import (
            save_focus_group_evaluation,
        )
        return save_focus_group_evaluation

    def test_tool_exists(self):
        tool = self._import_tool()
        assert callable(tool)

    def test_saves_go_recommendation(self):
        tool = self._import_tool()
        mock_ctx = mock.MagicMock()
        mock_ctx.state = {}
        ...
        result = tool(
            overall_score=7.5,
            recommendation="GO",
            top_strengths=["Strong visuals", "Good pacing", "Brand integration"],
            areas_for_improvement=["Minor color grading", "Audio sync", "CTA clarity"],
            iteration_number=1,
        ...
        assert mock_ctx.state["focus_group_evaluation"]["overall_score"] == 7.5
        assert mock_ctx.state["focus_group_evaluation"]["recommendation"] == "GO"
        assert len(mock_ctx.state["focus_group_evaluation"]["top_strengths"]) == 3
        assert len(mock_ctx.state["focus_group_evaluation"]["areas_for_improvement"]) == 3
        assert mock_ctx.state["focus_group_evaluation"]["iteration_number"] == 1

    def test_saves_nogo_recommendation(self):
        tool = self._import_tool()
        mock_ctx = mock.MagicMock()
        mock_ctx.state = {}
        ...
        assert mock_ctx.state["focus_group_evaluation"]["overall_score"] == 4.2
        assert mock_ctx.state["focus_group_evaluation"]["iteration_number"] == 2

    def test_overwrites_previous_evaluation(self):
        tool = self._import_tool()
        mock_ctx = mock.MagicMock()
        mock_ctx.state = {
        ...
        result = tool(
            overall_score=7.8,
            recommendation="GO",
            top_strengths=["Improved visuals", "Better pacing", "Strong CTA"],
            areas_for_improvement=["Minor tweaks"],
            iteration_number=2,
            tool_context=mock_ctx,
        )

        assert result["recommendation"] == "GO"
        assert mock_ctx.state["focus_group_evaluation"]["overall_score"] == 7.8
        assert mock_ctx.state["focus_group_evaluation"]["iteration_number"] == 2
...
class TestRootAgentIntegration:
    ...
    def test_root_prompt_has_iteration_protocol(self):
        from trends_and_insights_agent.prompts import ROOT_AGENT_INSTR
        assert "Iteration Protocol" in ROOT_AGENT_INSTR

    def test_root_prompt_has_go_nogo_logic(self):
        from trends_and_insights_agent.prompts import ROOT_AGENT_INSTR
        assert "GO" in ROOT_AGENT_INSTR
        assert "NO-GO" in ROOT_AGENT_INSTR

    def test_root_prompt_limits_iterations(self):
        from trends_and_insights_agent.prompts import ROOT_AGENT_INSTR
        assert "max 3 iterations" in ROOT_AGENT_INSTR or "iteration_number" in ROOT_AGENT_INSTR

    def test_root_prompt_has_step_6(self):
        from trends_and_insights_agent.prompts import ROOT_AGENT_INSTR
        assert "6." in ROOT_AGENT_INSTR

    def test_root_prompt_has_step_7(self):
        from trends_and_insights_agent.prompts import ROOT_AGENT_INSTR
        assert "7." in ROOT_AGENT_INSTR

    def test_focus_group_in_skills_init(self):
        from trends_and_insights_agent.skills import focus_group_evaluator_agent
        assert focus_group_evaluator_agent is not None
